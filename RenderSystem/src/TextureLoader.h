...
class TextureLoader final
{
     ...
     */
    static RS_EXPORT RStextureInfo create3Dvolume(uint32_t width, uint32_t height, uint32_t depth, RStextureFormat textureFormat);
     ...
     */
    static RS_EXPORT RStextureInfo readTexture(const char* filepath);
     ...
     * @return the texture info including the texel data in RGBA format.
     */
    static RS_EXPORT RStextureInfo readFromMemory(unsigned char* encodedTexData, uint32_t width, uint32_t height);
};
