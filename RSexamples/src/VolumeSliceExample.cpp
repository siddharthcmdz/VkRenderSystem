...
VolumeSliceExample::VolumeSliceExample() 
{

}

void VolumeSliceExample::init(const RSexampleOptions& eo, const RSexampleGlobal& globals) 
{
	...
	texInfo.texelFormat = RStextureFormat::tfUnsignedBytes;
	...
	std::cout << "Generating " << texInfo.width << " x " << texInfo.height << " x " << texInfo.depth << " noise texture..." << std::endl;
...
#define FRACTAL
				...
				data[x + y * texInfo.width + z * texInfo.width * texInfo.height] = static_cast<uint8_t>(floor(n * 255));
			}
		}
	}
...
void VolumeSliceExample::render(const RSexampleGlobal& globals) 
{
	auto& vkrs = VkRenderSystem::getInstance();
	vkrs.contextDrawCollections(globals.ctxID, globals.viewID);
}

void VolumeSliceExample::dispose(const RSexampleGlobal& globals) 
{
	//TODO: dispose collection and model data.
}
